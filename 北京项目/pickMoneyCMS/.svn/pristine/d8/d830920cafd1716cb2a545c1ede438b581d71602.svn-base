...
class AdModel extends BaseModel
{
    ...
    public function getAdListByPage($type ='',$status = '',$way = '',$keyword = '',$pagesize = 15)
    {
        ...
        $data['list'] = !empty($res->items()) ?  $res->items() : [];
        $data['page'] = $page;
...
//                $new_ad_config[$v['t_type']][] = $v;
     ...
     * @return array|false|\PDOStatement|string|\think\Collection
     */
    public function getNeedAdConfig($type = '')
    {
        if (empty($type)) {
            $list = Db::table($this->type_table)->where('status','=',1)->select();
            ...
            if (!empty($list)) {
                foreach ($list as $k=>$v) {
                $new_ad_config[$v['t_type']][$v['id']] = ['id'=>$v['id'],'title'=>$v['t_title']];
                }
            }
            //返回以广告管理配置类型为分组的数据
            return $new_ad_config;
    ...
    /**
     * 通过游戏展示类型获取一条游戏数据
     * @param int $game_type [三方游戏展示类型：1：首页红包；2：游戏大厅]
     * @return array|false|null|\PDOStatement|string|\think\Model
     */
    public function getThirdGameByType($game_type = 2)
    {
        $where[] = ['status','<>',9];           // 非删除的游戏
        $where[] = ['g_type','=',$game_type];   // 三方游戏展示类型
        return Db::table($this->third_game_table)->where($where)->find();
    }
