...
use app\api\model\CMApiSucLog;
use app\api\model\CMApiFailLog;
...
class BaseController extends Controller
{
	...
	public function __construct()
    {
        ...
        //动态根据全局变量判断：正式服务器关闭调试模式//20181023
        //新需求，测试服，部分账号测试运行，其他账号正常，所以用账号(100001-100010)进行控制，见85行//20181030
		//if (sys_ver() == 3) {
        //    $this->is_debug = 0;
        //}
        ...
        $this->request_type = isset($request_value['reqType']) ? intval($request_value['reqType']) : 0;//APP主动传1，WEB主动传2，默认0
        ...
        if (!empty($this->request_type)) {
            if (isset($request_value['action']) && isset($request_value['paramData']) && !empty($request_value['action'])) {//
                if (!is_sdk_encrypt($request_value['paramData'])) {
                    sdk_return('', 101, '参数数据异常');
                }
                $this->request_action = $request_value['action'];
                //var_dump($this->request_type);
                if ($this->request_type == 2) {
                    if (isset($request_value['uid']) && !empty($request_value['uid'])) {
                        //如果是测试的账户，允许直接PC端运行游戏，且auth=ab123456，方便开发人员测试//20181030
                        if($request_value['uid'] >= 100001 && $request_value['uid'] <= 100010){
                            $this->is_debug = 1;
                        }
                        $this->setParamWeb($request_value);
                    } else {
                        sdk_return('', 102, '非法请求数据(2)');
                    }
                } else {
                    $this->setParamApp($request_value);
                }
            } else {
                sdk_return('', 103, '非法请求数据(1)');
            }
        } else {
            $this->request_param = $request_value;
        }
    ...
    private function setParamApp($req)
    {
        ...
        if (!$req_decrypt) {
            ...
            if (isset($req_decrypt['auth']) && !empty($req_decrypt['auth'])) {
                $this->request_param = $req_decrypt;
