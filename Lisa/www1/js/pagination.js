...
(function(){
	...
	Pagination.prototype={
		render:function(obj){
			...
			if(obj.btnCount<5){obj.btnCount=5;}
			else if(obj.btnCount%2==0){obj.btnCount=obj.btnCount+1;}
			...
			$(that._wrapid).on('click','#pagination-prev',function(){
				if($(this).hasClass('pagination-disabled')){return;}
				that._currentPage--;
				$('#pagination-next').hasClass('pagination-disabled')&&$('#pagination-next').removeClass('pagination-disabled');
				if(that._currentPage==1){$('#pagination-prev').addClass('pagination-disabled');}
				else{$('#pagination-prev').removeClass('pagination-disabled');}
			...
			$(that._wrapid).on('click','#pagination-next',function(){
				...
				if(that._currentPage==that._btnNum){$('#pagination-next').addClass('pagination-disabled');}
				else{$('#pagination-next').removeClass('pagination-disabled')}
			...
			function isshowMore(){
				if(this._btnNum<=this._btnsValue){creatBtns.call(this,'none')}
				else{
					if(this._currentPage<=(this._btnsValue-1-this._halfbtns)){creatBtns.call(this,'after');}
					else if(this._currentPage>=this._btnNum-1-this._halfbtns){creatBtns.call(this,'before')}
					else{creatBtns.call(this,'all')}
					}
				}
			function creatBtns(ismorePosition){
					...
					var prevbtn='<span class="pagination-btn pagination-wz" id="pagination-prev">上一页</span>';
					var nextbtn='<span class="pagination-btn pagination-wz" id="pagination-next">下一页</span>'
					
				if(this._currentPage==1){
					firstbtn='<li class="pagenum pagination-btn pagination-current" data-num="1">1</li>';
					prevbtn='<span class="pagination-btn pagination-disabled" id="pagination-prev">上一页</span>'
				}
				if(this._currentPage==this._btnNum){
					...
					nextbtn='<span class="pagination-btn pagination-disabled" id="pagination-next">下一页</span>'
				}
				if(ismorePosition=='none'){
					for(var i=1;i<=this._btnNum;i++){
						if(i==this._currentPage){html+='<li class="pagenum pagination-btn pagination-current" data-num='+i+'>'+i+'</li>';}
						else{html+='<li class="pagenum pagination-btn" data-num='+i+'>'+i+'</li>';}
						}
					}
				...
				if(ismorePosition=="all"){
					...
					html+=ismore+lastbtn+"<div class='clear'></div>";
				}
			
				$(this._wrapid).html(prevbtn+'<ul class="pagination-wrap">'+html+'</ul>'+nextbtn);
			}
		},
		init:function(paginationObj){this.render(paginationObj);this.bindEvent();}
	}
