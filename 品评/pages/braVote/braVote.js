...
Page({
  ...
  createCode: function () {
    ...
    Req.POST(URL.GETCODE, {
      params: {
        file_name: 'cat' + app.globalData.catid+'.png',
  ...
  createShareImg: function () {
    ...
    // 绘制背景图
    ctx.drawImage('../../images/share_img_bg.png', 0, 0, w, h);
    // 绘制用户信息
    ctx.save();
    ctx.arc(140 * h/1334, 140 * h/1334, 60 * h/1334, 0, 2 * Math.PI);
    ctx.clip();
    var wxcode = that.data.codeUrl;
    var avatarUrl = that.data.avatarUrl;
    ctx.drawImage(avatarUrl, 80 * h/1334, 80 * h/1334, 120 * h/1334, 120 * h/1334);
    ctx.restore();
    ctx.save();
    ctx.setFillStyle('#353535');
    ctx.setFontSize(30 * h/1334);
    ctx.fillText(that.data.nickName, 220 * h / 1334, 134 * h / 1334);
    ctx.fillText("分享了一个"+that.data.catname+"的品牌投票哟~", 220 * h/1334, 174 * h/1334);
    ctx.save();
    ctx.setFillStyle('black');
    ctx.setFontSize(60 * h / 1334);
    ctx.fillText("十大投票", 295 * h / 1334, 450 * h / 1334);
    ctx.restore();
    ctx.setFillStyle('white');
    ctx.setFontSize(48 * h/1334);
    ctx.fillText("长按识别小程序", 400 * h/1334, 1090 * h/1334);
    ctx.fillText("立即参与！", 400 * h/1334, 1180 * h/1334);
    ctx.restore();
    // 绘制小程序码
    ctx.save();
    ctx.beginPath();
    ctx.setStrokeStyle('#00834a')
    ctx.arc(200 * h/1334, 1122 * h/1334, 130 * h/1334, 0, 2 * Math.PI);
    ctx.clip();
    ctx.stroke();
    ctx.drawImage(wxcode, 70 * h/1334, 992 * h/1334, 260 * h/1334, 260 * h/1334);
    ctx.closePath();
    ...
    Req.POST(URL.VOTEITEM, {
      ...
      success: function (res) {
        ...
        wx.setNavigationBarTitle({
          title: "中国" + res.data.catname + "十大品牌评选"
        });
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      imageUrl: '../../images/share_brand_img.png',
      success: function (res) {
