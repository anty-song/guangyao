...
{	
	...
	public function get_question_list()
	{
        ...
        // 广告url处理Jason 2018-10-30 start
        $memcache = get_memcache();
        $user_data = $memcache->get('user_data_' . $uid);
        $login_type = $user_data['login_type'];
        if ($login_type == 1) {  //安卓
            $get_answer_ad_popout['ad_url'] = $get_answer_ad_popout['ad_url1'];
        } elseif ($login_type == 2) {
            $get_answer_ad_popout['ad_url'] = $get_answer_ad_popout['ad_url2'];
        } else {
            $get_answer_ad_popout['ad_url'] = $get_answer_ad_popout['ad_url1'];
        }
        //Jason
        $get_answer_ad_popout['ad_title'] = !empty($get_answer_ad_popout['ad_info']) ? $get_answer_ad_popout['ad_info'] : '恭喜中奖，限时领取';
        unset($get_answer_ad_popout['ad_url1']);
        unset($get_answer_ad_popout['ad_url2']);
        //Jason end
    ...
    /**
     * 答题结束获取奖品 广告；广告+红包；广告+再玩一次
     * @Author Jason
     * @DateTime 2018-10-29
     */
	public function get_answer_last_reward()
    {
        $uid 			  = $this->uid;
        $g_id = 1;
        $status = 1;
        ...
        web_return($this->auth, $arr_data, 1, '获取成功');//Jason新增
    }
